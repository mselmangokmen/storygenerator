...
def generate_prompt(prompt,k=0,p=0.9,output_length=50,temperature=1,num_return_sequences=1,repetition_penalty=1.0):
    ...
    for generated_sequence_idx, generated_sequence in enumerate(output_sequences):
        ...
        if len(text)>1:
            ...
            return verbs, predicted_prompt 
        else:
            return None,None

model = GPT2LMHeadModel.from_pretrained('gpt2')
model.load_state_dict(torch.load("pretrained_models/gpt2_prompt_generator_verbs.pt"))

tokenizer = GPT2Tokenizer.from_pretrained('gpt2')
tokenizer.pad_token=tokenizer.eos_token
dataset_prep= PreparePromptsForGeneration(type='verb')
...
for i in range(1000):
    ...
    verbs, predicted_prompt = generate_prompt(prompt)
    print('generated')
    if verbs is not None and predicted_prompt is not None:
        ...
        prompt_dict= {'input_verbs': verbs  , 'predicted_prompt': predicted_prompt, 'target_prompt': target_prompt,   'index': index}
        prompt_dict_list.append(prompt_dict)

json_dict['prompts'] = prompt_dict_list
with open("datasets/predicted_prompts_with_verbs.json", "w") as outfile:
