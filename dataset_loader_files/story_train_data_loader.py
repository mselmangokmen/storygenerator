...
class StoryDataLoaderForGPT2(Dataset):
...
class PrepareStoriesForGPT2:  

    def __init__(self, batch_size = 10, max_seq_length= 1024)-> None:
        ...
        self.x_train_loader =  StoryDataLoaderForGPT2( self.x_train_tokenized)
        self.x_val_loader =  StoryDataLoaderForGPT2( self.x_val_tokenized) 
        self.dataloader = {'train': DataLoader(self.x_train_loader, batch_size=self.batch_size, shuffle=True,  ) 
                           , 'val': DataLoader(self.x_val_loader, batch_size=self.batch_size, shuffle=True,  ) }
    ...
    def create_train_set(self,stories):
        x_train= [] 
        for d in stories: 
            train_text = self.cleanpunctuation(d['target_prompt'])
            train_text += ' <sep> '+ self.cleanpunctuation(d['target_story'])
